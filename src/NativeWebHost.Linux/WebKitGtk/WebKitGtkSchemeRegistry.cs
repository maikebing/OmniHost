...
internal static class WebKitGtkSchemeRegistry
{
    ...
    public static void Register(nint webViewHandle, NativeWebHostOptions options)
    {
    ...
    private static void OnUriSchemeRequest(IntPtr request, IntPtr userData)
    {
        try
        {
            ...
            {
                FinishTextResponse(request, "Invalid NativeWebHost URI request.", "text/plain; charset=utf-8");
                return;
            }

            if (!Uri.TryCreate(requestUri, UriKind.Absolute, out var uri))
            {
                FinishTextResponse(request, $"Invalid NativeWebHost URI: {requestUri}", "text/plain; charset=utf-8");
                return;
            }
            ...
            {
                FinishTextResponse(request, $"No NativeWebHost scheme binding found for URI: {requestUri}", "text/plain; charset=utf-8");
                return;
            }
            ...
            {
                FinishTextResponse(request, $"NativeWebHost asset not found: {uri.AbsolutePath}", "text/plain; charset=utf-8");
                return;
            }
