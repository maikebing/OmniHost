...
/// <summary>
/// 组合运行 ASP.NET Core 本地服务和 NativeWebHost 桌面消息循环。
/// </summary>
public sealed class NativeWebApplicationHost : IAsyncDisposable
{
    private readonly NativeWebHostBuilder _desktopBuilder;
    private readonly IReadOnlyList<Action<NativeWebHostOptions, WebApplication>> _desktopConfigurations;
    private readonly Func<WebApplication, IDesktopApp?>? _desktopAppFactory;
    private int _hasRun;

    internal NativeWebApplicationHost(
        WebApplication web,
        NativeWebHostBuilder desktopBuilder,
        IReadOnlyList<Action<NativeWebHostOptions, WebApplication>> desktopConfigurations,
    ...
    private INativeWebHostApp BuildDesktopApp()
    {
