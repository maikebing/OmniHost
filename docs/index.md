---
title: OmniWebHost
---

# OmniWebHost

> Product short name: `Omni`  
> Planned technical family after the hosting refactor: `OmniHost`

**A cross-platform .NET desktop WebView hosting framework.**

OmniWebHost lets you build desktop applications powered by a web front-end, using whichever browser engine is available on the host platform — WebView2 on Windows, WKWebView on macOS, WebKitGTK on Linux, or CEF anywhere.

## Navigation

| | |
|--|--|
| [Getting Started](getting-started.md) | Install the package and create your first app |
| [Architecture](architecture.md) | How the components fit together |
| [JS Bridge](js-bridge.md) | Bidirectional C# ↔ JavaScript messaging |
| [Adapters](adapters.md) | Supported browser engines |
| [Roadmap](roadmap.md) | Release plan and milestones |

## Current Status

Version **0.1.0-preview.1** — architecture baseline.  
Windows / WebView2 is the first-class target for the MVP.
