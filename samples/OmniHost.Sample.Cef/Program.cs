...
using OmniHost;
using OmniHost.Cef;
using OmniHost.WinForms;

var app = OmniApp.CreateBuilder(args)
    .Configure(options =>
    {
        options.Title = "OmniHost CEF Sample";
    ...
    {
        options.Title = "OmniHost CEF Loading";
    ...
    {
        options.Title = "OmniHost CEF Secondary Window";
        options.StartUrl = "app://localhost/secondary.html?window=secondary";
        options.Width = 520;
        options.Height = 440;
        options.WindowStyle = OmniWindowStyle.Normal;
    })
    .UseAdapter(new CefAdapterFactory())
...
sealed class SampleApp : IWindowAwareDesktopApp
{
    ...
    public Task OnWindowStartAsync(OmniWindowContext window, CancellationToken cancellationToken = default)
    {
        window.Adapter.JsBridge.RegisterHandler("greet", payload =>
        {
            ...
            return Task.FromResult(
                $"\"Hello, {name}! This is CEF window '{window.WindowId}' on {Environment.MachineName}.\"");
        });
    ...
    private static OmniWindowDefinition CreateInspectorWindow()
    {
        ...
        {
            Title = "OmniHost CEF Inspector Window",
            CustomScheme = "app",
