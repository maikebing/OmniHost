...
using NativeWebHost.Sample.WebShowcase.Models;
using NativeWebHost.Sample.WebShowcase.Services;
...
webApp.MapGet("/api/blazor/summary", () =>
    Results.Ok(new BlazorSummary(
        ...
        new[]
        {
            ...
            "Typed JSON endpoint",
            "NativeWebHost desktop shell over localhost"
        })));
webApp.MapGet("/api/vue/tasks", () =>
    Results.Ok(new[]
    {
        ...
        new VueTask("Desktop wrapper", "NativeWebHost opens the SPA inside a native window", false),
    }));
...
webApp.MapRazorComponents<NativeWebHost.Sample.WebShowcase.Components.App>()
...
var app = NativeWebApp.CreateBuilder(args)
    .Configure(options =>
    {
        options.Title = "NativeWebHost Web Showcase";
...
sealed class WebShowcaseDesktopApp(string serverUrl) : IWindowAwareDesktopApp
{
    ...
    private readonly IReadOnlyDictionary<string, ScenarioDefinition> _scenarios =
        new Dictionary<string, ScenarioDefinition>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["third-party-site"] = new(
                ...
                "External site hosted directly in NativeWebHost.",
    ...
    private NativeWebWindowDefinition CreateScenarioWindow(ScenarioDefinition scenario)
    {
        ...
        return new NativeWebWindowDefinition($"{scenario.Id}-{index}", options);
    }
}
